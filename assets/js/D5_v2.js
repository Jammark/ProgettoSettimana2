...
for(var i in pets){
  console.log('elemento numero ' + (i + 1) + ' : ' + pets[i]);
}
...
*/
for(var i in pets){
  let index = pets.length - i - 1;
  console.log('elemento numero ' + (index + 1) + ' : ' + pets[index]);
}
...
cars.forEach(function(element){
  //let element = cars[i];
  let trim = element.trims[0];
  justTrims.push(trim);
});

console.log("just trims");
...
var indiceCiclo = 0;
while (indiceCiclo < numericArray.indexOf(32)){
  console.log(numericArray[indiceCiclo]);
  indiceCiclo++;
}
...
var posizioni = charactersArray.map(posizione);
